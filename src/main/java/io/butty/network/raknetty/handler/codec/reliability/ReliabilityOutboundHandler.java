...
import io.butty.network.raknetty.channel.event.ReceiptAcknowledgeEvent;
...
public class ReliabilityOutboundHandler extends ChannelOutboundHandlerAdapter {
    ...
    private final int[] orderedWriteIndex = new int[InternalPacket.NUMBER_OF_ORDERED_STREAMS];
    private final int[] sequencedWriteIndex = new int[InternalPacket.NUMBER_OF_ORDERED_STREAMS];
                ...
                while (totalSize < retransmissionBandwidth) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (currentTime - packet.actionTime >= 0) {
                            ...
                            Validate.isTrue(datagramSizes < mtuSize - Message.UDP_HEADER_SIZE);
                            sendList.add(packet);
                            packet.timeSent++;
